...
def get_total_square_value(location):
    site = gameMap.getSite(location)
    totalValue = (site.production * 26) - site.strength
    return totalValue
...
def acquire_targets(unitLocation):
    site = gameMap.getSite(unitLocation)
    targets = []
    for adjacent_location in get_adjacent_locations(unitLocation):
        adjacent_site = gameMap.getSite(adjacent_location)
        if site.strength > adjacent_site.strength and adjacent_site.owner != myID: # an adjacent space can be killed, mark it as a target
...
def directive_attack(unitLocation):
    ...
    if site.strength == 0:
        return STILL
    if site.production > (alliedProduction / len(alliedLocations)) and site.strength < (alliedStrength / len(alliedLocations)):
        return STILL
...
while True:
    ...
    for alliedLocation in alliedLocations: #loop through troops and give orders
        moves.append(Move(alliedLocation, get_move(alliedLocation)))
    sendFrame(moves)
